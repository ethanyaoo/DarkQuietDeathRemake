...
messageList[0] = "The batteries don't give much juice";
messageList[1] = "Conserve your battery juice";
messageList[2] = "The monsters will chase you to death";
messageList[3] = "There is no escape";
...
function PlaceRandomBattery()
{
	...
	{
		if (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objEnemy) 
			&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objSolid))
				&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objPlayer))
					&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objBattery))
						&& countSquareNeighbors(randX, randY) > 1 && countDiagonalNeighbors(randX, randY) > 1)
		{
...
function PlaceEnemy()
{
	var randX = irandom_range(3, _width - 1);
	var randY = irandom_range(3, _height - 1);
...
function PlaceSigns()
{
	var randX = irandom_range(1, _width - 1);
	var randY = irandom_range(1, _height - 1);
			...
			&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objSolid))
				&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objPlayer))
					&& (!place_meeting((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), objBattery))
						&& countSquareNeighbors(randX, randY) > 1 && countDiagonalNeighbors(randX, randY) > 1)
		{
			var textboxMessage = messageList[messageCount];
			
			with (instance_create_layer((CELL_WIDTH / 2) + (randX * SECTION_WIDTH), (CELL_HEIGHT / 2) + (randY * SECTION_HEIGHT), "Items", objTextboxSign))
			{
				textMessage = textboxMessage;
...
function DrawGrid()
{
	...
	var yOffset = 48 // QUICK FIX TO OFFSET SHROUD GRID NOT PLACING CORRECTLY
